...
surf = [1, 2]
parser.add_argument("-s", "--surface", required = True, choices = surf, default = 2,\
                    type = int, help = "Build surface 1 (bottom) or surace 2 (top)")

parser.add_argument("-str", "--strain", action = "store_true",\
...
parser.add_argument("-rep", "--repeat", type = int, default = 3,\
...
itf.exportSurface(idx = opt.index, z = opt.repeat, format = opt.format, filename = opt.output,\
                  surface = opt.surface, verbose = opt.verbose, alt_base = opt.alt_base,\
